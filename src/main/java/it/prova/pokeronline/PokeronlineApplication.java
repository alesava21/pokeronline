	...
	public void run(String... args) throws Exception {
		...
		if (ruoloServiceInstance.cercaPerDescrizioneECodice("Classic User", Ruolo.ROLE_CLASSIC_USER) == null) {
			ruoloServiceInstance.inserisciNuovo(
					Ruolo.builder().descrizione("Classic User").codice(Ruolo.ROLE_CLASSIC_USER).build());
		}

		if (ruoloServiceInstance.cercaPerDescrizioneECodice("Special User", Ruolo.ROLE_SPECIAL_USER) == null) {
			ruoloServiceInstance.inserisciNuovo(
					Ruolo.builder().descrizione("Special User").codice(Ruolo.ROLE_SPECIAL_USER).build());
		}
		...
		if (utenteServiceInstance.findByUsername("admin") == null) {
			Utente admin = Utente.builder().nome("mario").cognome("rossi").username("admin").password("admin")
					.dataRegistrazione(LocalDate.of(2000, 8, 10)).esperienzaAccumulata(3).creditoAccumulato(500)
					.build();
		...
		if (utenteServiceInstance.findByUsername("user") == null) {
			Utente classicUser = Utente.builder().nome("roberto").cognome("bianchi").username("user").password("user")
					.dataRegistrazione(LocalDate.of(2001, 10, 20)).esperienzaAccumulata(6).creditoAccumulato(1000)
					.build();
			classicUser.ruoli()
		...
		if (utenteServiceInstance.findByUsername("userSpecial") == null) {
			Utente specialUser = Utente.builder().nome("alessandro").cognome("sava").username("userSpecial")
					.password("userSpecial").dataRegistrazione(LocalDate.of(1990, 1, 27)).esperienzaAccumulata(10)
					.creditoAccumulato(10000).build();
			specialUser.ruoli()
					.add(ruoloServiceInstance.cercaPerDescrizioneECodice("Classic User", Ruolo.ROLE_CLASSIC_USER));
		...
		if (utenteServiceInstance.findByUsername("user1") == null) {
			Utente user1 = Utente.builder().nome("michele").cognome("esposito").username("user1").password("user1")
					.dataRegistrazione(LocalDate.of(2010, 6, 10)).esperienzaAccumulata(0).creditoAccumulato(0).build();
			user1.ruoli().add(ruoloServiceInstance.cercaPerDescrizioneECodice("Classic User", Ruolo.ROLE_CLASSIC_USER));
